// lib/features/series/widgets/episode_comments_bottom_sheet.dart
// REPURPOSED from channel comments - Comments for series episodes
...
import 'package:textgb/features/series/models/episode_comment_model.dart';
import 'package:textgb/features/series/providers/episode_comments_provider.dart';
...
class EpisodeCommentsBottomSheet extends ConsumerStatefulWidget {
  final SeriesEpisodeModel episode;
  final VoidCallback? onClose;

  const EpisodeCommentsBottomSheet({
    super.key,
  ...
  @override
  ConsumerState<EpisodeCommentsBottomSheet> createState() => _EpisodeCommentsBottomSheetState();
}

class _EpisodeCommentsBottomSheetState extends ConsumerState<EpisodeCommentsBottomSheet>
    with TickerProviderStateMixin {
  ...
  Widget _buildEpisodeInfo() {
    return Container(
      ...
      child: Row(
        children: [
          // Episode thumbnail or series image
          Container(
            width: 40,
            height: 40,
            decoration: BoxDecoration(
            ...
            child: widget.episode.seriesImage.isNotEmpty
                ? ClipRRect(
                    borderRadius: BorderRadius.circular(7),
                    child: Image.network(
                      widget.episode.seriesImage,
                      width: 40,
                      height: 40,
                      fit: BoxFit.cover,
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(
                      ' • ${timeago.format(widget.episode.createdAt.toDate())}',
                      style: const TextStyle(
                        color: _mediumGray,
                        fontSize: 12,
                      ),
  ...
  Widget _buildCommentsList() {
    final commentsStream = ref.watch(episodeCommentsStreamProvider(widget.episode.id));
  ...
  List<CommentGroup> _groupCommentsByReplies(List<EpisodeCommentModel> comments) {
  ...
  Widget _buildEnhancedCommentItem(EpisodeCommentModel comment, {bool isReply = false}) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          ElevatedButton(
            onPressed: () {
              ref.invalidate(episodeCommentsStreamProvider(widget.episode.id));
            },
  ...
  void _likeComment(EpisodeCommentModel comment) {
    final currentUser = ref.read(currentUserProvider);
    if (currentUser == null) return;

    final isLiked = comment.likedBy.contains(currentUser.uid);
    ref.read(episodeCommentActionsProvider)
  ...
  Future<void> _sendComment() async {
    ...
    final success = await ref
        .read(episodeCommentActionsProvider)
        .addComment(
...
class CommentGroup {
  final EpisodeCommentModel mainComment;
  final List<EpisodeCommentModel> replies;
