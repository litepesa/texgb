// lib/features/users/widgets/payment_required_banner_widget.dart
...
class PaymentRequiredBannerWidget extends ConsumerStatefulWidget {
  ...
  const PaymentRequiredBannerWidget({
    super.key,
...
class _PaymentRequiredBannerWidgetState extends ConsumerState<PaymentRequiredBannerWidget>
  ...
  Widget build(BuildContext context) {
    // Watch the current user's payment status
    final currentUser = ref.watch(currentUserProvider);
    final hasPaid = currentUser?.hasPaid ?? true;

    // Only show if user has NOT paid (or forceShow is true)
    if (!widget.forceShow && hasPaid) {
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                const Text(
                                  'Pay activation fee to post on the marketplace',
                                  style: TextStyle(
                                    color: Colors.white,
                                    fontSize: 13,
...
class PaymentRequiredBannerCard extends ConsumerStatefulWidget {
  ...
  const PaymentRequiredBannerCard({
    super.key,
    this.forceShow = false,
    this.onTap,
  });
