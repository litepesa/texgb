...
class _SingleVideoScreenState extends ConsumerState<SingleVideoScreen>
  ...
  Future<void> _openDirectMessage(VideoModel? video) async {
    if (video == null) {
      debugPrint('No video available for direct messaging');
      return;
    }

    // Check if user is authenticated
    final canInteract = await _checkUserAuthentication('send direct messages');
    if (!canInteract) return;
    ...
    try {
      ...
      if (chatId != null) {
        // Navigate to chat screen using router
        if (mounted) {
      ...
      // Resume video playback after navigation
      ...
      child: Column(
        children: [
          ...
          _buildRightMenuItem(
            ...
            label: 'Inbox',
            onTap: () => _openDirectMessage(currentVideo),
          ),
