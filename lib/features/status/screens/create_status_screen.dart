...
class _CreateStatusScreenState extends State<CreateStatusScreen> {
  ...
  Future<void> _publishStatus() async {
    if (_selectedMedia.isEmpty) {
      showSnackBar(context, 'Please add photos or video to your status');
      return;
    }
    ...
    if (_isVideo) {
      ...
      await statusProvider.uploadMediaStatus(
        ...
        onError: (error) {
          showSnackBar(context, 'Error posting status: $error');
        },
      );
    } else if (_selectedMedia.length == 1) {
      // Upload a single image status
      await statusProvider.uploadMediaStatus(
        currentUser: currentUser,
        file: _selectedMedia.first,
      ...
      // Upload multiple images status
      await statusProvider.uploadMultiImageStatus(
        currentUser: currentUser,
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Your status will be visible to everyone',
                          style: TextStyle(
                            color: accentColor,
