...
class _StatusDetailScreenState extends State<StatusDetailScreen> with SingleTickerProviderStateMixin {
  ...
  bool _isExiting = false; // Track exit animation
  ...
  // Move to next status with improved animation
  void _nextStatus() {
    if (_isPaused) return; // Don't advance if paused
    ...
    if (_currentIndex < widget.statuses.length - 1) {
      ...
      // Last status, animate exit and go back
      _animateAndExit();
    }
  }
  
  // Move to previous status with improved animation
  void _previousStatus() {
    if (_isPaused) return; // Don't go back if paused
    ...
    if (_currentIndex > 0) {
      ...
      // First status, animate exit and go back
      _animateAndExit();
    }
  }
  
  // Animate exit and pop back
  void _animateAndExit() {
    setState(() {
      _isExiting = true;
      _isPaused = true; // Pause during exit animation
    });
  ...
  // Toggle pause state
  void _togglePause() {
    if (_isExiting) return; // Don't toggle if exiting
  ...
  void _startActionTimeout() {
    Future.delayed(const Duration(seconds: 3), () {
      if (mounted && _isShowingActions && !_isPaused && !_isExiting) {
        setState(() {
                    ...
                    Positioned(
                      ...
                      child: Row(
                        children: [
                          ...
                          // Close button with improved exit handling
                          IconButton(
                            icon: const Icon(
                              Icons.close,
                              color: Colors.white,
                            ),
                            onPressed: _animateAndExit,
                          ),
                        ],
                      ),
                    ),
  ...
  // Build the status content based on type with improved TikTok-style videos
    ...
    switch (status.type) {
      ...
      case StatusType.video:
        return StatusVideoPlayer(
          ...
          isPaused: _isPaused,
          useTikTokStyle: true, // Use TikTok style for better viewing
        );
      default:
        return const Center(
          child: Text(
