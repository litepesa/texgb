// lib/features/video_reactions/widgets/video_reaction_message_bubble.dart
// COPIED: Exact same UI as chat message bubble but for video reaction messages
...
class VideoReactionMessageBubble extends StatelessWidget {
  ...
  const VideoReactionMessageBubble({
    super.key,
  ...
  bool _isMediaMessage() {
    // Check for original video reaction messages
  ...
  Widget _buildMessageContent(
    ...
    // Check if this is an original video reaction message
    if (message.isOriginalReaction && message.videoReactionData != null) {
  ...
  // Build video reaction content (the original video reaction message)
  Widget _buildVideoReactionContent(
    BuildContext context,
    ModernThemeExtension modernTheme,
    ChatThemeExtension chatTheme,
  ) {
    final videoReactionData = message.videoReactionData!;

    return VideoReactionBubbleWidget(
      videoReaction: videoReactionData,
  ...
  Widget _buildInnerReplyPreview(BuildContext context, ModernThemeExtension modernTheme) {
    // Create a mock VideoReactionMessageModel for the replied-to message using the reply fields
    final repliedToMessage = VideoReactionMessageModel(
      messageId: message.replyToMessageId ?? '',
      chatId: message.chatId,
      senderId: message.replyToSender ?? '',
      content: message.replyToContent ?? '',
      type: _getReplyMessageType(),
      status: MessageStatus.sent,
      timestamp: DateTime.now(),
      mediaUrl: _getReplyMediaUrl(),
    );

    return VideoReactionReplyPreview(
  ...
  // Simple video content like image bubble - just thumbnail with play button
