...
class HttpMomentsRepository implements MomentsRepository {
  ...
  Future<List<MomentModel>> getFeed({int page = 1, int limit = 20}) async {
    try {
      final response = await _httpClient.get(
        '/api/v1/moments?page=$page&limit=$limit',
      );
  ...
  Future<List<MomentModel>> getUserMoments(
    ...
    try {
      final response = await _httpClient.get(
        '/api/v1/moments/user/$userId?page=$page&limit=$limit',
      );
  ...
  Future<MomentModel?> getMoment(String momentId) async {
    try {
      final response = await _httpClient.get('/api/v1/moments/$momentId');

      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return MomentModel.fromJson(data['moment'] as Map<String, dynamic>);
  ...
  Future<bool> deleteMoment(String momentId) async {
    try {
      final response = await _httpClient.delete('/api/v1/moments/$momentId');
  ...
  Future<bool> unlikeMoment(String momentId) async {
    try {
      final response = await _httpClient.delete('/api/v1/moments/$momentId/like');
  ...
  Future<MomentCommentModel> commentOnMoment(
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final data = jsonDecode(response.body);
        return MomentCommentModel.fromJson(data['comment'] as Map<String, dynamic>);
      } else {
  ...
  Future<bool> deleteComment(String commentId) async {
    try {
      final response = await _httpClient.delete('/api/v1/moments/comments/$commentId');
  ...
  Future<List<MomentCommentModel>> getComments(
    ...
    try {
      final response = await _httpClient.get(
        '/api/v1/moments/$momentId/comments?page=$page&limit=$limit',
      );
  ...
  Future<List<MomentLikerModel>> getLikes(
    ...
    try {
      final response = await _httpClient.get(
        '/api/v1/moments/$momentId/likes?page=$page&limit=$limit',
      );
